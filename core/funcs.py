...
async def search(message: Message) -> Optional[Song]:
    ...
    if reply:
        if reply.text:
            ...
            if not media:
                return None
            lel = await message.reply_text("`Tentando baixar...`")
            file = await reply.download(
                progress=progress_bar,
                progress_args=("Baixando...", lel, time.time()),
            )
            await lel.delete()
            return Song(
...
async def progress_bar(current, total, ud_type, msg, start):
    ...
    if round((now - start) % 3) == 0 or current == total:
        ...
        current_message = f"**Baixando...** `{round(percentage, 2)}%`\n`{progressbar}`\n**Baixado**: `{humanbytes(current)}` | **Total**: `{humanbytes(total)}`\n**Velocidade**: `{humanbytes(speed)}/s` | **Tempo**: `{time_to_complete}`"
        if msg:
            try:
